...
import { admissionNavLinks, admissionsContent } from '../data/admissionsContent';

const AdmissionPage = ({ pageKey }) => {
  const { slug } = useParams();
  const location = useLocation();

  const key = pageKey || slug;
  const page = admissionsContent[key];

  if (!page) {
    return <Navigate to="/" replace />;
  }

  const currentPath = pageKey === 'scholarships' ? '/scholarships' : location.pathname;
